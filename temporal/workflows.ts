...
export async function syncDatabaseStats(
	connectionId: string,
	...
	try {
		...
		if (!connection || connection.status !== "ACTIVE") {
			log.warn("Connection not active, aborting sync", {
				connectionId,
		...
		log.info("Finished syncDatabaseStats workflow successfully", {
			connectionId,
		});
		return { success: true };
	} catch (error) {
		log.error("syncDatabaseStats workflow failed", {
			connectionId,
			error: String(error),
		});
		try {
			...
			log.error("Failed to update connection status to ERROR", {
				connectionId,
				error: String(e),
			});
		}
...
export async function generateSuggestions(
	connectionId: string,
): Promise<{ success: boolean; suggestions?: number; error?: string }> {
	log.info("Starting generateSuggestions workflow", { connectionId });
	try {
		const connection = await getConnection(connectionId);

		if (!connection) {
			log.warn("Connection not found, aborting suggestion generation", {
				connectionId,
			});
		...
		if (slowQueries.length === 0) {
			log.info("No slow queries found, skipping suggestion generation", {
				connectionId,
			});
		...
		log.info("Finished generateSuggestions workflow successfully", {
			connectionId,
		});
		return { success: true, suggestions: suggestions.length };
	} catch (error) {
		log.error("generateSuggestions workflow failed", {
			connectionId,
			error: String(error),
		});
		return { success: false, error: String(error) };
